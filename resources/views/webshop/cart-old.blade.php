    ...
    @if(Cart::count() > 0)
        ...
        <div class="row">
            @foreach ($cart as $item)
                <?php
                    $product = \App\Models\Product::findByNumber($item->id);
                    $name = (strlen($item->name) > 50 ? substr($item->name, 0, 47) . "..." : $item->name);

                    if ($product->isAction()) {
                        $nettoPrice = number_format($product->real_price * $item->qty, 2, ".", "");
                    } else {
                        $nettoPrice = number_format(($product->real_price * ((100-$product->discount) / 100)) * $item->qty, 2, ".", "");
                    }

                    $total += (double) $nettoPrice;
                ?>
                ...
                <form action="/cart/update" method="POST" class="col-md-4 col-sm-6">
                    ...
                    <div class="panel panel-primary">
                        <div class="panel-heading">
                            ...
                            <a href="/product/{{ $product->number }}" class="panel-heading-link">
                                {{ $product->short_name }}
                            </a>
                        </div>
                        <table class="table">
                            ...
                            <tr>
                                ...
                                <td class="col-xs-6">
                                    <div class="input-group">
                                        <input type="text" class="form-control" placeholder="{{ $item->qty }}"
                            ...
                            <tr>
                                ...
                                <td><span class="glyphicon glyphicon-euro"></span> {{ number_format($nettoPrice, 2, ".", "") }}</td>
                            </tr>
                            </tbody>
                        </table>
                    </div>
